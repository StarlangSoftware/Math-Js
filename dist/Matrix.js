(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "./Vector", "./Eigenvector"], factory);
    }
    ...
    class Matrix {
        ...
        multiplyWithVectorFromLeft(v) {
            ...
            return new Vector_1.Vector(result);
        }
        /**
         ...
         */
        multiply(m) {
            const result = new Matrix(this.row, m.col);
            for (let i = 0; i < this.row; i++) {
         ...
         */
        partial(rowStart, rowEnd, colStart, colEnd) {
            const result = new Matrix(rowEnd - rowStart + 1, colEnd - colStart + 1);
         ...
         */
        clone() {
            const copy = new Matrix(this.row, this.col);
            for (let i = 0; i < this.row; i++) {
                for (let j = 0; j < this.col; j++) {
        ...
        choleskyDecomposition() {
            const b = new Matrix(this.row, this.col);
            for (let i = 0; i < this.row; i++) {
        ...
        characteristics() {
            ...
            for (let i = 0; i < this.row; i++) {
                if (d[i] > 0) {
                    result.push(new Eigenvector_1.Eigenvector(d[i], v.getColumnVector(i)));
                }
            }
